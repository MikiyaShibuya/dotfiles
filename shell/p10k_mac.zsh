# Generated by Powerlevel10k configuration wizard on 2020-06-26 at 02:51 JST.
...
() {
  ...
  function prompt_example() {
    ...
    if [ "$(uname)" = 'Darwin' ]; then
        # For Apple silicon
        MAKER=`sysctl machdep.cpu.brand_string | awk '{print $2}'`
        if [ "$MAKER" = 'Apple' ]; then
            # Show what architecture mode used
            ARCH=`uname -m`
            p10k segment -b 7 -f 0 -t "${ARCH}"
        fi
    fi
  }
